    ...
    public void run(GuildMessageReceivedEvent event, String[] args) {
        ...
        if (author != owner) { //Checks if the command executor is the owner
            ...
            if (mentionedMember.getRoles().contains(warnTwoRole)) { //3. Kiks
                ...
                message.reply(MessageUtils.defaultEmbed("Bro u literally killed him"
                , "<:catLoading:853319548874129488> Succesfully banned " + mentionedMember.getAsMention()
                + " from the MC server! Better u get a ban revoke at "
                + owner.getAsMention(), false).build()).queue();
                ...
                message.reply(MessageUtils.defaultEmbed("What have you done this time?"
                , "<:lmao:853559874390720532> Succesfully warned " + mentionedMember.getAsMention()
                + "!\nWoah thats the 2nd warn, bro be careful with what ur doing!", false).build()).queue();

            } else { //1. Kiks
                event.getGuild().addRoleToMember(mentionedMember, warnOneRole).queue(); //Add the 1. KIKS role

                message.reply(MessageUtils.defaultEmbed("Got'em"
                , "<:lol:853271360884047873> Succesfully warned " + mentionedMember.getAsMention()
                + "!", false).build()).queue();
            }
        } 
    }
