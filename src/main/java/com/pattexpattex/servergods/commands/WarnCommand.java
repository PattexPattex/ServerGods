    ...
    public void run(GuildMessageReceivedEvent event, String[] args) {
        ...
        if (author != owner) { //Checks if the command executor is the owner
            ...
            if (mentionedMember.getRoles().contains(warnTwoRole)) { //3. Kiks
                ...
                , "<:catLoading:853319548874129488> Succesfully banned " + mentionedMember.getAsMention() + " from the MC server! Better u get a ban revoke at " + owner.getAsMention(), mentionedMember.getUser().getAvatarUrl(), null).build()).complete();
